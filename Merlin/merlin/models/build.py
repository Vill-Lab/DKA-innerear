...
from merlin.models import i3res, pmti3res
...
class pmtImageEncoder(ImageEncoder):
    def __init__(self, ImageEmbedding: bool = False, prompt_length : int = 48):
        super().__init__(ImageEmbedding=ImageEmbedding)
        resnet = torchvision.models.resnet152(pretrained=True)
        self.i3_resnet = pmti3res.pmtI3ResNet(
            copy.deepcopy(resnet), class_nb=1692, conv_class=True, ImageEmbedding=self.ImageEmbedding, prompt_length = prompt_length
        )

    def forward(self, image, feats):
        if self.ImageEmbedding:
            contrastive_features = self.i3_resnet(image, feats=feats)
            return contrastive_features
        else:
            contrastive_features, ehr_features = self.i3_resnet(image)
            return contrastive_features, ehr_features
...
class MerlinArchitecture(nn.Module):
    ...
    def forward(self, image, text=None, feats=None):
        if self.ImageEmbedding and text is None:
            if feats is None:
                image_features = self.encode_image(image)
            else:
                image_features = self.encode_image(image, feats)
